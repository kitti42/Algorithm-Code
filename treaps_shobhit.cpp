...
template<class T,class R> class treap
{
	...
	public:
		...
		;
		class iterator:public ::iterator<bidirectional_iterator_tag,key_type,size_t,key_type&,key_type>
		{	
			node *current;
			bool is_active;
			public:
...
int main()
{
	...
	pair<treap<int,random>,treap<int,random> > treaps=a.split(50);
	
	
	treap<int,random> x=merge(treaps.first,treaps.second,50);
	
	for(treap<int,random>::iterator i=x.begin();i!=x.end();i++)
		cout<<*i<<endl;
	...
	for(int i=0;i<1000;i++)
	{
		...
		treap<int,random>::iterator j=a.end();
	...
	for(map<size_t,size_t>::iterator i=heights.begin();i!=heights.end();i++)
